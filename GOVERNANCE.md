...
This document defines project governance for the StreamsHub project.
...
A component owner is responsible for development of a specific subproject or component within the StreamsHub organisation.
Such components might be represented by a separate GitHub repository within the StreamsHub organization (e.g. `flink-sql-runner`) or by a subdirectory in a GitHub repository (e.g. `./docs` in `console`).
...
A maintainer is actively responsible for the overall development of the whole StreamsHub project and all its components.
...
* They will be added to the collaborators list of the StreamsHub repository with "Write" access
* After 6 months they will be given "Admin" access to the StreamsHub repository.

Maintainers can be removed by a vote.
If the vote succeeds:
* Their name will be removed from the `MAINTAINERS` file.
* They will be removed to the collaborators list of the StreamsHub repository
...
* They will be removed to the collaborators list of the StreamsHub repository, if any.
...
If a maintainer or component owner has not made any contributions to the project for a period of 6 months, any other maintainer may propose that they be moved to the corresponding emeritus status.
The voting rules for removal are the same as for removing maintainers or component owners.

Contributions to the project are defined as any of following activities performed on any of the StreamsHub owned GitHub repositories:

* Opening issues or PRs
* Commenting on issues or PRs
* Reviewing or approving PRs

Before a vote is called the maintainer or component owner in question should be contacted (via the email address defined in the relevant MAINTAINERS or COMPONENT-OWNERS files) to check whether they wish to remain active or weather they have a valid reason (such as a medical issue) for their inactivity. 
...
Proposals should cover any changes to the StreamsHub project which might significantly impact its users or the project direction.
